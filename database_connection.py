...
def run_query(sql_query, params=None):
    ...
    try:
        ...
        # Open an explicit connection from the engine and wrap the query in text()
        # This completely resolves the UserWarning and SQL parsing crashes
        with engine.connect() as conn:
            query_obj = text(sql_query)
            df = pd.read_sql(query_obj, con=conn, params=params)
