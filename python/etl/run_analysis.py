...
# All queries run against PostgreSQL (uganda_health_db database, public schema).
...
def safe_int(value, default=0):
    """
    Safely convert a value to int, falling back to `default` when the
    value is None/NaN (e.g. a SQL SUM()/AVG() over zero or all-NULL rows).
    """
    if value is None:
        return default
    try:
...
def q11_top_districts_by_cases(year=None, top_n=5):
    ...
    sql = f"""
        ...
        GROUP BY d.district_name, d.region,d.estimated_population, d.is_urban
...
def q12_trend_by_disease(disease_name):
    ...
    sql = """
        ...
        WHERE c.condition_name ILIKE :disease_pattern
        GROUP BY dt.year
        ORDER BY dt.year ASC;
    """
    print(f"\n[Query 12] Trend for Disease: {disease_name}...")
    return run_query(sql, params={"disease_pattern": f"%{disease_name}%"})
...
def q13_facility_summary(facility_id):
    ...
    sql = f"""
        ...
        GROUP BY h.hospital_key, h.hospital_name, h.hospital_id, h.hospital_type, 
                 h.city, h.state, h.country, d.district_name, d.region;
    """
...
def q17_district_summary():
    ...
    sql = """
        SELECT 
            ...
            COALESCE(SUM(d.estimated_population), 0) AS total_population,
            ROUND(COALESCE(AVG(d.estimated_population), 0)::numeric, 0) AS avg_population_per_district,
            COALESCE(SUM(CASE WHEN d.is_urban THEN d.estimated_population ELSE 0 END), 0) AS urban_population,
            COALESCE(SUM(CASE WHEN NOT d.is_urban THEN d.estimated_population ELSE 0 END), 0) AS rural_population
...
def make_page1(df_monthly, df_conditions, df_admission, df_tests):
    """
    Page 1: Yearly Revenue | Top Conditions | Admission Costs | Test Results
    """
    # ── Check if we have any data for this page ──────────────────────────────
    has_data = False

    if (
        (df_monthly is not None and not df_monthly.empty and df_monthly["total_revenue"].max() > 0)
        or (df_conditions is not None and not df_conditions.empty and df_conditions["total_cost"].max() > 0)
        or (df_admission is not None and not df_admission.empty and df_admission["avg_cost"].max() > 0)
        or (df_tests is not None and not df_tests.empty and df_tests["total_tests"].sum() > 0)
    ):
        has_data = True
    ...
    fig.suptitle(
        ...
        "Yearly Revenue  ·  Conditions  ·  Admission Types  ·  Test Results",
        fontsize=14, fontweight="bold", y=0.98
    )
    gs = gridspec.GridSpec(2, 2, figure=fig, hspace=0.35, wspace=0.30)

    # ── Chart 1: Yearly Revenue Trend (line) ──────────────────────────────────
    ...
    try:
        if df_monthly is not None and not df_monthly.empty and df_monthly["total_revenue"].max() > 0:
            ...
            ax1.plot(
                df_yearly["year"],
                df_yearly["total_revenue"],
                marker="o", linewidth=2.5, markersize=8, color="#2C3E50"
            )
            ax1.set_title("Yearly Revenue Trend", fontweight="bold", fontsize=12)
            ...
            max_rev = df_yearly["total_revenue"].max()
            for _, row in df_yearly.iterrows():
                ax1.text(
                    row["year"], row["total_revenue"] + max_rev * 0.02,
                    f"${row['total_revenue']:,.0f}",
                    ha="center", fontsize=8, fontweight="bold", rotation=45
                )
        else:
            ax1.text(0.5, 0.5, "No yearly revenue data available", 
                     ha="center", va="center", transform=ax1.transAxes, fontsize=12)
            ax1.set_title("Yearly Revenue Trend", fontweight="bold", fontsize=11)
    except (KeyError, TypeError, ValueError) as e:
        print(f"⚠️ Error creating Chart 1: {e}")
        ax1.text(0.5, 0.5, "Yearly revenue data unavailable", 
                 ha="center", va="center", transform=ax1.transAxes, fontsize=12)
    ...
    plt.savefig("../../outputs/reports/hospital_analysis_page1.png", dpi=150, bbox_inches="tight")
    print("\n  Page 1 saved: outputs/reports/hospital_analysis_page1.png")
    plt.show()
...
def make_page2(df_insurance, df_demographics, df_medications, df_gender):
    ...
    if (
        (df_insurance is not None and not df_insurance.empty and df_insurance["patients"].max() > 0)
        or (df_demographics is not None and not df_demographics.empty and df_demographics["patient_count"].max() > 0)
        or (df_medications is not None and not df_medications.empty and df_medications["prescription_count"].max() > 0)
        or (df_gender is not None and not df_gender.empty and df_gender["total_visits"].max() > 0)
    ):
        has_data = True
                ...
                if not age_summary.empty and age_summary["patient_count"].max() > 0:
                    ...
                    for bar in bars6:
                        ...
                        ax6.text(
                            ...
                            f"{safe_int(h):,}", ha="center", fontsize=9, fontweight="bold"
                        )
            ...
            if not df_medications_clean.empty and df_medications_clean["prescription_count"].max() > 0:
                ...
                for bar in bars7:
                    ...
                    ax7.text(
                        w + max_count * 0.01,
                        bar.get_y() + bar.get_height() / 2,
                        f"{safe_int(w):,}", va="center", fontsize=9, fontweight="bold"
                    )
    ...
    plt.tight_layout()
    os.makedirs("../../outputs/reports", exist_ok=True)
    plt.savefig("../../outputs/reports/hospital_analysis_page2.png", dpi=150, bbox_inches="tight")
    print("  Page 2 saved: outputs/reports/hospital_analysis_page2.png")
    plt.show()
...
def make_page3(df_doctors, df_demographics, df_conditions, df_admission):
    ...
    if (
        df_doctors is not None
        and not df_doctors.empty
        and df_doctors["patients_handled"].max() > 0
    ) or (
        df_conditions is not None
        and not df_conditions.empty
        and (
            df_conditions["cost_per_visit"].max() > 0
            or df_conditions["patient_count"].max() > 0
        )
    ) or (
        df_admission is not None
        and not df_admission.empty
        and df_admission["visits"].max() > 0
    ):
        has_data = True
    ...
    try:
        if df_doctors is not None and not df_doctors.empty:
            df_doctors_clean = df_doctors.dropna(subset=['patients_handled', 'doctor_name']).copy()

            if not df_doctors_clean.empty:
                if df_doctors_clean["doctor_name"].duplicated().any() and "specialty" in df_doctors_clean.columns:
                    df_doctors_clean["doctor_label"] = (
                        df_doctors_clean["doctor_name"].astype(str)
                        + " ("
                        + df_doctors_clean["specialty"].astype(str)
                        + ")"
                    )
                else:
                    df_doctors_clean["doctor_label"] = df_doctors_clean["doctor_name"].astype(str)

                df_doc_sorted = df_doctors_clean.sort_values("patients_handled", ascending=True)

                bars9 = ax9.barh(
                    df_doc_sorted["doctor_label"],
                    df_doc_sorted["patients_handled"],
                    color=COLOURS["doctors"][: min(len(df_doc_sorted), len(COLOURS["doctors"]))]
                        * (len(df_doc_sorted) // len(COLOURS["doctors"]) + 1),
                    height=0.6
                )
                ...
                print(f"⚠️ Chart 9: df_doctors had {len(df_doctors)} row(s) but all were dropped "
                      f"by dropna(['patients_handled','doctor_name']) — check for NULLs in those columns.")
            ...
            print("⚠️ Chart 9: q6_doctors() returned an empty DataFrame. This usually means the "
                  "JOIN between fact_patient_visits.doctor_key and doctor.doctor_key found no "
                  "matches — check that doctor_key is populated (not NULL) on fact_patient_visits "
                  "and that its values/type match doctor.doctor_key.")
                ...
                if not df_cond2.empty and df_cond2["patient_count"].max() > 0:
                    ...
                    for bar in ax11.patches:
                        ...
                        ax11.text(
                            ...
                            f"{safe_int(h):,}", ha="center", fontsize=8
                        )
    ...
    plt.tight_layout()
    os.makedirs("../../outputs/reports", exist_ok=True)
    plt.savefig("../../outputs/reports/hospital_analysis_page3.png", dpi=150, bbox_inches="tight")
    print("  Page 3 saved: outputs/reports/hospital_analysis_page3.png")
    plt.show()
...
def make_page4(df_top_districts, df_disease_trend, df_high_risk, df_district_summary):
    ...
    # ── Check if we have any data for this page ──────────────────────────────
    has_data = (
        (df_top_districts is not None and not df_top_districts.empty and df_top_districts["total_cases"].max() > 0)
        or (df_disease_trend is not None and not df_disease_trend.empty and df_disease_trend["total_cases"].max() > 0)
        or (df_high_risk is not None and not df_high_risk.empty and df_high_risk["total_cases"].max() > 0)
        or (df_district_summary is not None and not df_district_summary.empty)
    )
            ...
            if not df_disease_trend_clean.empty and df_disease_trend_clean["total_cases"].max() > 0:
                ...
                for i, (_, row) in enumerate(df_disease_trend_clean.iterrows()):
                    ax14.text(
                        ...
                        f"{safe_int(row['total_cases']):,}", ha="center", fontsize=9, fontweight="bold"
                    )
            else:
    ...
    try:
        ...
        📊 DISTRICT OVERVIEW
        ─────────────────────
        Total Districts      : {safe_int(stats['total_districts']):,}
        Total Regions        : {safe_int(stats['total_regions']):,}
        
        🏙️ Urban/Rural Split
        ─────────────────────
        Urban Districts      : {safe_int(stats['urban_districts']):,}
        Rural Districts      : {safe_int(stats['rural_districts']):,}
        
        👥 Population
        ─────────────────────
        Total Population     : {safe_int(stats['total_population']):,}
        Avg per District     : {safe_int(stats['avg_population_per_district']):,}
        Urban Population     : {safe_int(stats['urban_population']):,}
        Rural Population     : {safe_int(stats['rural_population']):,}
        """
            ...
            ax16.text(0.5, 0.5, "No district summary data available", 
                      ha="center", va="center", transform=ax16.transAxes, fontsize=12)
    except (AttributeError, KeyError, TypeError, ValueError, IndexError) as e:
    ...
    plt.tight_layout()
    os.makedirs("../../outputs/reports", exist_ok=True)
    plt.savefig("../../outputs/reports/hospital_analysis_page4.png", dpi=150, bbox_inches="tight")
    print("  Page 4 saved: outputs/reports/hospital_analysis_page4.png")
    plt.show()
...
def print_summary(df1, df2, df4, df5, df6, df8, df9, df10, 
    ...
    print("  Star Schema · uganda_health_db database · public schema")
    print(dline)
    ...
    for _, row in df1.iterrows():
        ...
        print(f"  {row['year']}-{row['month_name'][:3]:<5} "
              ...
              f"{bar}  ({safe_int(row['total_patients'])} patients)")
    ...
    for _, row in df2.head(6).iterrows():
        ...
        print(f"  {row['condition_name']:<20} "
              ...
              f"{bar}  ({safe_int(row['patient_count'])} patients)")
    ...
    for _, row in df4.iterrows():
        ...
        print(f"  {row['insurance_provider']:<20} "
              f"{safe_int(row['patients']):>4} patients  "
    ...
    for _, row in df5.iterrows():
        ...
        print(f"  {row['admission_type']:<12} "
              f"{safe_int(row['visits']):>5} visits  "
    ...
    for _, row in df6.head(5).iterrows():
        ...
        print(f"  {str(row['doctor_name'])[:22]:<24} "
              f"{safe_int(row['patients_handled']):>3} patients  "
    ...
    for _, row in df8.head(5).iterrows():
        ...
        print(f"  {str(row['medication_name'])[:22]:<24} "
              f"{safe_int(row['prescription_count']):>4} prescriptions  "
    ...
    for _, row in df9.iterrows():
        ...
        print(f"  {row['test_result']:<14} "
              f"{safe_int(row['total_tests']):>5} tests  "
              f"({pct:5.1f}%)  {bar}")
    ...
    for _, row in df10.iterrows():
        ...
        print(f"  {row['gender']:<10} "
              f"{safe_int(row['total_visits']):>6} visits  "
              f"${row['avg_billing']:>10,.2f} avg  "
              f"({pct:5.1f}%)  {bar}")
    ...
    if df_top_districts is not None and not df_top_districts.empty:
        ...
        for _, row in df_top_districts.head(5).iterrows():
            ...
            print(f"  {row['district_name']:<20} "
                  f"{safe_int(row['total_cases']):>5} cases  "
                  f"${row['total_revenue']:>12,.2f} revenue  "
                  f"{bar}  ({safe_int(row['unique_patients'])} patients)")
    ...
    if df_high_risk is not None and not df_high_risk.empty:
        ...
        for _, row in df_high_risk.head(3).iterrows():
            ...
            print(f"  {risk_symbol} {row['region']:<15} "
                  f"{safe_int(row['total_cases']):>6} cases  "
                  f"{row['risk_level']:<8}  "
    ...
    if df_district_summary is not None and not df_district_summary.empty:
        ...
        print(line)
        print(f"  Total Districts        : {safe_int(stats['total_districts']):,}")
        print(f"  Total Regions          : {safe_int(stats['total_regions']):,}")
        print(f"  Urban Districts        : {safe_int(stats['urban_districts']):,}")
        print(f"  Rural Districts        : {safe_int(stats['rural_districts']):,}")
        print(f"  Total Population       : {safe_int(stats['total_population']):,}")
        print(f"  Avg Population/District: {safe_int(stats['avg_population_per_district']):,}")
        print(f"  Urban Population       : {safe_int(stats['urban_population']):,}")
        print(f"  Rural Population       : {safe_int(stats['rural_population']):,}")
...
def save_summary_to_file(df1, df2, df4, df5, df6, df8, df9, df10,
    ...
    """
    os.makedirs("../../outputs/reports", exist_ok=True)
    report_filepath = "../../outputs/reports/hospital_analysis_summary.txt"
    ...
    with open(report_filepath, "w", encoding="utf-8") as f:
        ...
        f.write("  Star Schema · uganda_health_db database · public schema\n")
        f.write(f"{dline}\n")
        ...
        if df17 is not None and not df17.empty:
            stats = df17.iloc[0]
            f.write(f"\n{line}\n  📊 DISTRICT OVERVIEW\n{line}\n")
            f.write(f"  Total Districts        : {safe_int(stats['total_districts']):,}\n")
            f.write(f"  Total Regions          : {safe_int(stats['total_regions']):,}\n")
            f.write(f"  Urban Districts        : {safe_int(stats['urban_districts']):,}\n")
            f.write(f"  Rural Districts        : {safe_int(stats['rural_districts']):,}\n")
            f.write(f"  Total Population       : {safe_int(stats['total_population']):,}\n")
            f.write(f"  Avg Population/District: {safe_int(stats['avg_population_per_district']):,}\n")
            f.write(f"  Urban Population       : {safe_int(stats['urban_population']):,}\n")
            f.write(f"  Rural Population       : {safe_int(stats['rural_population']):,}\n")

        f.write(f"\n{dline}\n")

    print("✅ Text summary report saved: outputs/reports/hospital_analysis_summary.txt")
...
def run_full_analysis():
    ...
    print("  Database: uganda_health_db  |  Schema: public  |  Star Schema")
    print("=" * 58)
    print("\nRunning queries...\n")
    ...
    print("  outputs/reports/hospital_analysis_page1.png")
    print("  outputs/reports/hospital_analysis_page2.png")
    print("  outputs/reports/hospital_analysis_page3.png")
    print("  outputs/reports/hospital_analysis_page4.png")
